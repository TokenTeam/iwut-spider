...
class Spider(private val httpClientProvider: ISpiderHttpClientProvider) : ISpider {
    ...
    override fun run(info: SpiderInfo, environment: Map<String, String>): Map<String, String> {
        ...
        val httpClient = httpClientProvider.create(info.engine)
    ...
    private fun runStep(task: SpiderTaskInfo, context: MutableMap<String, String>, client: ISpiderHttpClient) {
        ...
        val res = if (task.method == SpiderMethod.GET) {
            ...
            client.post(url, reqHeaders, payload, task.payload?.type ?: SpiderPayloadType.TEXT, successCode)
        }

        // Parse content
        task.content?.let {
            if (it.value != null) {
                IContentParser.create(it.type).also { it.context = context }.parse(res.content, it.pattern, it.value)
            }
        }
